...
class scara():
    def __init__(self):
        ...
        # self.home_pose = pose(49.315, 123)
        self.home_pose = pose(49.315, 130)
    ...
    def init(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.target_received and self.sucker_task == "Grab":
                self.armState.action = "Grab"
                self.armState.situation = "Execute"
                self.controller(self.target_pose)
                if self.task_received:
                    if not self.domain_error_break == 1:
                        while not self.sucker_feedback == "Executing" and not rospy.is_shutdown():
                            self.sucker_publish(self.sucker_task, self.target_pose)
                            print("sucker executing ...")
                            pass
                        while (not self.sucker_feedback == "Success" and not self.sucker_feedback == "Fail") and not rospy.is_shutdown():
                            self.sucker_publish(self.sucker_task, self.target_pose)
                            pass                    
                        self.sucker_publish("Hold", self.target_pose)
                        self.controller(self.home_pose)
                self.armState.action = "Grab"
                self.armState.situation = self.sucker_feedback
                if self.domain_error_break == 1:
                    print ("domain errorrrrrrrrrrr")
                    self.armState.situation = "Fail"
                self.target_received = 0
                self.task_received = 0
                self.sucker_task = "Hold"
                self.domain_error_break = 0
            ...
            if self.sucker_task == "Release":
                self.armState.action = "Release"
                self.armState.situation = "Execute"
                self.controller(self.release_pose)
                if self.task_received:
                    while not self.sucker_feedback == "Executing" and not rospy.is_shutdown():
                        self.sucker_publish(self.sucker_task, self.release_pose)
                        print("sucker executing ...")
                        pass
                    while (not self.sucker_feedback == "Success" and not self.sucker_feedback == "Fail") and not rospy.is_shutdown():
                        pass                    
                    self.sucker_publish("Hold", self.release_pose)
                    self.controller(self.home_pose)
                self.armState.action = "Release"
                self.armState.situation = self.sucker_feedback
                self.target_received = 0
